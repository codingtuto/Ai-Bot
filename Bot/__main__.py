# Copyright New-dev0 & Coding Team
import re
import aiohttp
...
HI_STRINGS = ["Salut", "Bonjour", "Coucou"]
    ...
    for message in msgs:
        if message["author"] == "bot":
            ...
            if message.get("linkUrl"):
                buttons.append([Button.url("⚡ Résultats sur Google", message["linkUrl"])])
...
async def set_language(event):
    bts = Buttons[0].copy()
    bts.append([Button.inline("Suivant ▶", "btsh"), Button.inline("Annuler ❌", "cncl")])
    await event.reply("🌐 Sélectionnez une langue...", buttons=bts)
...
async def click_next(event):
    ...
    try:
        ...
        bt = Buttons[0].copy()
    if val == 0:
        bt.append([Button.inline("Suivant ▶", "btsh"), Button.inline("Annuler ❌", "cncl")])
    else:
        bt.extend(
            [
                [
                    Button.inline("◀ Précédent", f"btshp{val}"),
                    Button.inline("Suivant ▶", f"btshn{val}"),
                ],
                [Button.inline("Annuler ❌", "cncl")],
            ]
        )
...
async def set_lang(event):
    ...
    await event.edit(f"👊 Langue changé avec succée en {name} !")


@client.on(NewMessage(pattern="^/source$"))
async def source_cmd(event):
    await event.reply("Code Source bientôt disponnible.")
...
if Var.OWNER_ID:
    ...
    async def save_db(event):
        ...
        if msg:
            ...
            return await event.respond(f"Enregistrée [vers]({msglink})!")
        await event.respond("Quelques choses ne vas pas!")
    ...
    async def broadcast(event):
        try:
            ...
            if event.is_reply:
                ...
                return await event.respond(
                    "Reponds avec cette message puis envoie moi le message general..."
                )
        users = JSONDB["users"]
        total = len(users)
        success = 0
        for user in users:
            try:
                ...
                LOG.info(f"Mailing: {user} | {er}")
                users.remove(user)
            except Exception as er:
                LOG.error(er)
        await event.reply(
            f"**Mailing complet!**\n**Succées:** {success}\n"
            + f"**Erreur:** {total - success}"
        )

    @client.on(NewMessage(pattern="^/stats$", from_users=Var.OWNER_ID))
    async def stats_message(event):
        await event.reply(f"**Utilisateurs totales:** {len(JSONDB['users'])}")


with client:
    LOG.info(f"Ok! envoie /start à @{SELF.username}...")
